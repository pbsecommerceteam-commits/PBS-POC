/* ─────────────────────────────────────────────────────────────────────────
   ...
   assembled by the two query functions at the bottom:
...
const rowRng = (key, tag, id) => rng(hash(key + "|" + tag + "|" + id));
...
function series(seed, n, start, end, amp, digits) {
  const r = rng(seed);
...
const swing = { "7d": 0.25, "4w": 0.5, "12w": 1, "6m": 1.5, "12m": 2.2 };

function productFor(p, key) {
  ...
  return {
    ...
    category: p.category,
    retailer: p.retailer,
    retailerName: (retailers.find((x) => x.id === p.retailer) || {}).name,
...
function withSalesMetrics(q, key) {
...
function snapshot(retailer, period) {
  ...
  const kpi = (id, label, unit, vals, target, digits) => ({
  ...
  const out = {
    ...
    stockByRetailer: retailers.slice(1).map((rt, i) => {
    ...
    shareByRetailer: retailers.slice(1).map((rt, i) => {
    ...
    categoryShare: categories.map((c, i) => {
    ...
    competitors: competitorBrands.map((c, i) => {
    ...
    contentCoverage: contentAttributes.map((a, i) => {
    ...
    reviewThemes: reviewThemes.map((t, i) => {
    ...
    products: pool,
    retailerPerformance: retailers.slice(1).map((rt, i) => {
    ...
    categoryPerformance: categories.map((c, i) => {
      const rr = rowRng(key, "categoryPerf", c);
      const inCat = pool.filter((p) => p.category === c);
      const avg = (f, d) => (inCat.length ? round(inCat.reduce((a, p) => a + f(p), 0) / inCat.length, d) : 0);
...
function deriveInsights(s) {
  const sos = s.kpis.find((k) => k.id === "sos");
  const topCat = s.categoryPerformance.slice().sort((a, b) => b.sos - a.sos)[0];
  const lowAvail = s.products.filter((p) => p.inStockRate < 95);
  const weakContent = s.products.filter((p) => p.contentScore < 80);
  const ownAvgPrice = s.products.length
    ? s.products.reduce((a, p) => a + p.price, 0) / s.products.length : 0;
  /* Like-for-like: our own price index against the cheapest rival's index. */
  const ownIndex = s.products.length
    ? s.products.reduce((a, p) => a + p.priceIndex, 0) / s.products.length : 1;
  const byIndex = s.competitors.slice().sort((a, b) => a.priceIndex - b.priceIndex);
  const cheapest = byIndex[0];
  const undercut = round(((ownIndex - cheapest.priceIndex) / ownIndex) * 100, 1);
  const risers = s.competitors.slice().sort((a, b) => b.delta - a.delta);
  const rising = risers.find((c) => c.id !== cheapest.id) || risers[0];
...
export function fetchSnapshot({ retailer = "all", period = "12w" } = {}) {
  return new Promise((resolve) => setTimeout(() => resolve(snapshot(retailer, period)), LATENCY));
}
...
function shelfData(retailer, period) {
  const key = retailer + "|" + period;
  const seed = hash(key);
  const labels = labelsFor(period);
  const n = labels.length;
  ...
  const last = (a) => a[a.length - 1];
  const first = (a) => a[0];
  const avg = (arr, f, d) => (arr.length ? round(arr.reduce((a, x) => a + f(x), 0) / arr.length, d == null ? 1 : d) : 0);
  ...
  const compPrices = competitorBrands.map((c, i) => {
...
function salesData(retailer, period) {
  const key = retailer + "|" + period;
  ...
  const rivals = competitorBrands.slice(0, 3).map((c, i) => {
  ...
  const rankOf = (id) => rankOrder.indexOf(id) + 1;
  ...
  const shelfKpi = (id) => shelf.kpis.find((k) => k.id === id) || { delta: 0, value: 0 };
  const signals = [
  ...
  const SIGNAL_FOCUS = { "Availability": "avail", "Search visibility": "rank", "Content score": "content", "Buy box share": "avail" };
  const diagnosis = {
    sales: growth,
...
export function fetchSales({ retailer = "all", period = "12w" } = {}) {
  return new Promise((resolve) => setTimeout(() => resolve(salesData(retailer, period)), LATENCY));
}

export function fetchShelf({ retailer = "all", period = "12w" } = {}) {
  return new Promise((resolve) => setTimeout(() => resolve(shelfData(retailer, period)), LATENCY));
}

export function fetchProduct(id, { retailer = "all", period = "12w" } = {}) {
  return new Promise((resolve, reject) => setTimeout(() => {
    const base = catalog.find((p) => p.id === id);
    if (!base) return reject(new Error("Product not found: " + id));
    const key = retailer + "|" + period;
    const p = withSalesMetrics(withShelfMetrics(productFor(base, key)), key);
