...
export default function DigitalShelfSummary() {
  ...
  const visFooter = [
    ...
    { label: "Buy box share", value: buy.value + "%", color: buy.value >= buy.target ? "var(--status-positive-fg)" : "var(--status-neutral-fg)" },
  ];
            ...
            return (
              <Card key={r.id} interactive selected={active} padding="16px 18px" onClick={() => { if (!active) { setRetailer(r.id); toast("Digital shelf scoped to " + r.name + "."); } }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {[["Visibility", pct(r.visibility)], ["Availability", pct(r.availability)], ["Price index", r.priceIndex.toFixed(1)], ["Content", String(r.content)], ["Rating", r.rating.toFixed(2)]].map(([l, v]) => (
      ...
      <Card padding="20px 22px 10px">
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="sl-table">
            <thead><tr>
              {[["category", "Category", "left", 180], ["visibility", "Search visibility", "right"], ["availability", "Availability", "right"], ["content", "Content", "right"], ["priceIndex", "Price index", "right"], ["overall", "Overall score", "right", 140]].map(([k, label, align, mw]) => (
