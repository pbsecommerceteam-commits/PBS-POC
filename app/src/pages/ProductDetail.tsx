...
import { kpiCard, cell, table, money, signedPct, deltaColor, pct } from "../lib/format";
...
export default function ProductDetail() {
  ...
  const kpis = [
    ...
    { id: "instock", label: "In Stock %", unit: "%", value: p.inStockRate, target: 98, delta: Number((p.inStockRate - t.stock[0]).toFixed(1)), spark: t.stock },
    { id: "rating", label: "Average Rating", unit: "", value: p.rating, target: 4.5, delta: Number((p.rating - t.rating[0]).toFixed(2)), spark: t.rating },
    { id: "content", label: "Content Score", unit: "/100", value: p.contentScore, target: 95, delta: 0, spark: t.stock.map((_v: number, i: number) => Math.round(p.contentScore - 4 + (i / labels.length) * 4)) },
  ];

  const maxStars = Math.max(...detail.reviewMix.map((m: any) => m.count));
  const lo = Math.min(...t.price), hi = Math.max(...t.price);
  const maxSalesTrend = Math.max(...detail.sales.trend);
  ...
  const stockChart = barChart({ id: "d-stock", title: "Stock Availability", subtitle: "In-stock rate at this retailer", badge: "Target 98%",
  ...
  const salesChart = lineChart({ id: "d-sales", title: "Sales Trend", subtitle: "Estimated sales for this SKU over the period",
    labels, lo: 0, hi: maxSalesTrend * 1.15, ticks: [0, maxSalesTrend * 0.5, maxSalesTrend * 1.1], fmt: money, hideLegend: true, span: "1 / -1",
    series: [{ name: "Sales", values: detail.sales.trend }],
    footer: [
      { label: "Sales this period", value: money(detail.sales.value), color: "var(--status-positive-fg)" },
      { label: "Previous period", value: money(detail.sales.previous), color: "inherit" },
      { label: "Growth", value: signedPct(detail.sales.growth), color: deltaColor(detail.sales.growth) },
      { label: "Category growth", value: signedPct(detail.sales.categoryGrowth), color: "inherit" },
    ] }, hover, onEnter);

  const facts = [
    { label: "Search visibility", value: p.searchVisibility + "%", sub: "Rank #" + p.searchRank + " on the primary term" },
    { label: "Current price", value: "$" + p.price.toFixed(2), sub: "Index " + (p.priceIndex * 100).toFixed(0) + " vs. category" },
    { label: "Availability", value: p.inStockRate.toFixed(1) + "%", sub: p.stockStatus },
    { label: "Reviews", value: p.reviews.toLocaleString(), sub: p.rating.toFixed(2) + " average rating" },
    { label: "Estimated sales", value: money(detail.sales.value), sub: detail.sales.units.toLocaleString() + " units this period" },
    { label: "Sales growth", value: signedPct(detail.sales.growth), sub: "Category " + signedPct(detail.sales.categoryGrowth) },
    { label: "Share of category", value: pct(detail.sales.contribution), sub: "Of tracked category sales" },
  ];
  ...
  const retailerPerfTable = table("Retailer performance", "The same SKU measured at every monitored retailer",
    [{ label: "Retailer", align: "left" }, { label: "Rank", align: "right" }, { label: "Price", align: "right" },
     { label: "In Stock", align: "right" }, { label: "Rating", align: "right" }, { label: "Content", align: "right" }, { label: "Listing", align: "left" }],
  ...
  const salesByRetailerTable = table("Sales by retailer", "Where this SKU's volume is coming from",
    [{ label: "Retailer", align: "left" }, { label: "Sales", align: "right" }, { label: "Units", align: "right" }, { label: "Growth", align: "right" }],
    detail.sales.byRetailer.map((r: any) => ({ cells: [
      cell(r.retailer), cell(money(r.sales), { align: "right", strong: true }),
      cell(r.units.toLocaleString(), { align: "right" }),
      cell(signedPct(r.growth), { align: "right", color: deltaColor(r.growth) }),
    ] })));
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,430px),1fr))", gap: "var(--app-gap)" }}>
        ...
        <ChartCard c={salesChart} onLeave={onLeave} />
      </div>

      <DataTable t={retailerPerfTable} />
      <DataTable t={salesByRetailerTable} />
