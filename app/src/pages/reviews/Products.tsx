...
export const REVIEWS_COLUMNS: Column<Product>[] = [
  { key: "name", label: "Product", minWidth: 280, sortable: true, render: (p) => <ProductCell id={p.id} name={p.name} sku={p.id.toUpperCase()} meta={`${p.category} · ${p.retailerName}`} imageUrl={p.imageUrl} urlFailed={p.urlFailed} noClamp />, csv: (p) => `${p.id.toUpperCase()} - ${p.name}` },
